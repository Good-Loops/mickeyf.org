...
**Release is not yet approved.** This reconciliation made no production changes,
dispatched no workflow, created no PR, and ran no build, install or application
test. It does not transfer an earlier image-specific risk acceptance or certify
the current feature branch from default-branch scan results.
...
| S6 | Fixed in branch; not merged | 13 of 14 open default-branch dependency alerts map to fixes already in branch locks: eight `fast-uri` alerts (3.1.7), two `qs` alerts (6.16.0), three `xmldom` alerts (0.8.15 / 0.9.12). No new dependency patch follows merely from the push warning. This is an alert-to-lock comparison, not a fresh npm audit. |
...
| S9 | Blocked; current-branch CI | At the inspected head: no open PR, check runs, commit statuses, Actions runs or branch CodeQL analyses. The latest successful branch PR workflow is from September 1 at `4d8b6389`, not this candidate. Obtain current checks; do not rerun accepted manual gameplay to fill this gap. |
...
GitHub currently reports 14 default-branch alerts: nine high, five medium.
...
The sole remaining GitHub alert in the reviewed locks is #287,
...
1. Run the existing non-deploying `pr-ci.yml` workflow on the current branch,
   with authorization to dispatch it. It has `workflow_dispatch` support and
   contains no deployment job. A PR is still needed for the eventual merge and
   current CodeQL/required-check evidence. This reconciliation dispatched none.
...
Read-only Git/GitHub state, manifest/source/history comparisons, selected Cloud
Run/trigger/secret-IAM/project-role reads and official upstream metadata were
used. Secret payloads were not read; only configuration metadata and secret
references were inspected. No fresh SQL query,
image build/scan, full transitive-IAM audit, npm audit, application test or
physical-device session is claimed. `git diff --check` covers the documentation
